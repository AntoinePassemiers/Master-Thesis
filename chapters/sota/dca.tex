\section{Direct Coupling Analysis}
    ...
    \subsection{Potts model} \label{potts}
        ...
        homologous proteins. Spatial contacts between residues can thus be inferred  % Shouldn't it be the other way around? It's weird to say that 3D structure has an impact on AA sequence since AAs induce 3D structure [Anfinsen]
    ...
    \subsection{Exact inference is hard}
        ...
        Given a multiple sequence alignment containing $B$ sequences, a na\"\i ve approach would be to maximize its log-likelihood:  % $B$ sequences or $M$ sequences?
